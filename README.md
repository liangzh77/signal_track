...
- Glassmorphism dashboard with source/status/direction filters, project list, detail cards, recent input feed, embedded report snapshots, logic blocks, and SVG return curves.
...
run daily checks, scan the recent input feed, publish the dashboard, refresh
instrument masters, run market data coverage/smoke checks, refresh backend
health, store the API key locally in the browser, and jump to the dashboard.
...
If you put the source and note on one line, separate them with `;`, `；`, `|`,
`,` or `，`, for example `信息源：Alpha Desk，00700.HK 做多，观察广告`.
